...
export class AccountingComponent implements OnInit {
  ...
  onRowClick(event: any, invoice: Invoice) {
    const isPrintButton = (event.target as HTMLElement).closest('.p-button-print');
    if (!isPrintButton) {
      this.viewInvoiceDetails(invoice);
    }
  }

  printInvoice(invoice: Invoice, event: Event) {
    event.stopPropagation();
    
    this.accountingService.getInvoiceById(invoice.id).subscribe({
      next: (fullInvoice) => {
        const printStyles = `
          <style>
      ...
      error: (error) => {
        this.messageService.add({
          severity: 'error',
          summary: 'خطأ',
          detail: 'فشل في تحميل تفاصيل الفاتورة للطباعة'
        });
      }
    });
  }
  ...
  getRawMaterialName(materialId: number): string {
    const invoice = this.invoices.find(inv => 
      inv.purchase.raw_materials.some(rm => rm.id === materialId)
    );
    if (!invoice) return 'غير معروف';
    
    const material = invoice.purchase.raw_materials.find(rm => rm.id === materialId);
